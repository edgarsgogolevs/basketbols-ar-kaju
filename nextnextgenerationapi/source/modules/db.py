...
class Db:
    ...
    def select(self, query: str, data: Union[dict, None] = None) -> list:
    ...
    def update_dict(self, table: str, data: dict, where: dict) -> int:
        ...
        where_q = " AND ".join([f"{column} = %s" for column in where.keys()])
    ...
    def insert_or_update_dict(self, table: str, upd: dict, where: dict) -> int:
        cursor = self.get_cursor()
        columns = ", ".join(
            [f"{column} = %s" for column in upd.keys()]
            + [f"{column} = %s" for column in where.keys()]
        )
        update_columns = ", ".join([f"{column} = %s" for column in upd.keys()])
